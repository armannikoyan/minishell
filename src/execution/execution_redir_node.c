...
int scan_and_process_heredocs(t_ast_node *node, t_hash_table *ht, int *counter)
{
    ...
    if (node->type == HEREDOC_NODE)
    {
        ...
        // Prepare the limiter (remove quotes if necessary)
        ...
        while (true)
        {
            ...
            // (Optional: Add variable expansion here if limiter wasn't quoted)
        ...
        // Change the node type to REDIRECT_IN
        // Replace the filename (limiter) with the path to the temp file
        free(node->u_data.redir.filename);
        node->u_data.redir.filename = ft_strdup(filename);
        node->type = REDIRECT_IN_NODE; // Treat it as a normal file input now
    }
