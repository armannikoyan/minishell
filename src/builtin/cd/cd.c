...
static char *get_path(const char *cd_path, const char *path) {
  ...
  if (cd_path[0] != '/') {
    ...
    if (getcwd(cwd, sizeof(cwd)) == NULL)
      return (print_error("cd: change_dir: get_path_comb: getcwd", false),
              NULL);
...
int ft_cd(const int argc, char **argv, t_hash_table *ht,
  ...
  if (argc == 1) {
    ...
    if (ht_get(ht, "HOME") == NULL)
      return (
          print_error("cd: environment variable 'HOME' doesn't exist\n", true),
          2);
  ...
  if (argc == 2 && argv[1][0] != '\0') {
    if (strcmp("-", argv[1]) == 0) {
      ...
      if (target == NULL)
        return (
            print_error("cd: environment variable 'OLDPWD' is empty\n", true),
            2);
