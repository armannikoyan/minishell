...
//
// Arguments:
// - res: writable result buffer (single allocation, size >= original path
// length + 1).
// - res_len: pointer to current length of data in `res` (bytes used, without
// NUL).
// - dir: pointer to the directory component inside original path (not
// NUL-terminated).
...
static int append_and_validate(char *res, size_t *res_len, const char *dir,
                               size_t dir_len) {
  int c;

  if (*res_len > 1)
    res[(*res_len)++] = '/';
  memcpy(res + *res_len, dir, dir_len);
  *res_len += dir_len;
  res[*res_len] = '\0';

  c = check_access(res);
  if (!(c & EXISTS_BIT) || (c & STAT_FAIL_BIT) || !(c & DIR_BIT) ||
      !(c & EXEC_BIT))
    return 1;
  return 0;
}
...
static int remove_and_validate(char *res, size_t *res_len) {
  ...
  remove_last_dir(res);
  *res_len =
      strlen(res); // Update length to prevent heap corruption on next append
...
static const char *copy_until_dotdot(char *res, size_t *res_len,
                                     const char *p) {
  const char *start;
...
char *normalize_and_resolve_path(const char *path) {
  ...
  if (!path)
    return NULL;
  res = (char *)calloc(strlen(path) + 1, sizeof(char));
  if (res == NULL)
    return NULL;
