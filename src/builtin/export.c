...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/27 01:15:33 by lvarnach          #+#    #+#             */
/*   Updated: 2026/01/27 01:43:06 by lvarnach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char *argv)
{
	...
	if (argv[len] == '=' && argv[len + 1] == '\0')
	{
		...
		if (ht_get(ht, argv) == NULL)
			...
			(ht_get(ht, argv)->is_local = false, ht_update_value(ht, argv, ""));
	}
	else if (argv[len] == '=')
	{
...
int	ft_export(int argc, char **argv, t_hash_table *ht)
{
	...
	if (key == NULL)
		return (print_error("export: malloc", false), BUILTIN_ERROR);
